...
namespace arbioApp.Modules.Principal.DI._2_Documents
{
    public partial class frmEditDocument : DevExpress.XtraEditors.XtraForm
    {
        ...
        public void InitializeGrid(GridControl gc, string DoPiece)
        {
            ...
            gvLigneEdit.Columns["DL_PoidsNet"].DisplayFormat.FormatType = DevExpress.Utils.FormatType.Numeric;
            gvLigneEdit.Columns["DL_PoidsNet"].DisplayFormat.FormatString = "N2";
        ...
        private void checkEdit_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (check != null)
            {
                ...
                var values = gvLigneEdit.GetListSourceRowCellValue(row, "DL_PoidsNet");
                ...
                {
                    MessageBox.Show($"La saisie du poids est obligatoire!!!", "Erreur", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
    ...
    (short?)numExpedition, expeditIntitule, dO_Coord01, numeroDepot, DoTaxe1, newStatut, newDoDate);
        ...
        private void Editor_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (rowHandle >= 0)
            {
                ...
                var values = gvLigneEdit.GetListSourceRowCellValue(rowHandle, "DL_PoidsNet");
                    ...
                    remiseObj != DBNull.Value && tauxTVAObj != DBNull.Value)
                {
                    ...
                    gvLigneEdit.CustomUnboundColumnData += (sender, e) =>
                    {
                        if (e.IsGetData)
                        {
                            ...
                            {
                                var value = gvLigneEdit.GetListSourceRowCellValue(e.ListSourceRowIndex, "DL_PoidsNet");

                                if (!string.IsNullOrEmpty(Convert.ToString(value)) && txt_prix.Text!="" && txt_poids.Text != "") { 
                            ...
                            {
                                var value = gvLigneEdit.GetListSourceRowCellValue(e.ListSourceRowIndex, "FRET");
        ...
        private void frmEditDocument_Load_1(object sender, EventArgs e)
        {
            ...
            gvLigneEdit.CustomUnboundColumnData += (sender, e) =>
            {
                if (e.IsGetData)
                {
                    if (e.Column.FieldName == "FRET")
                    {
                        if (txt_prix.Text != "" && txt_poids.Text != "" && gvLigneEdit.GetListSourceRowCellValue(e.ListSourceRowIndex, "DL_PoidsNet").ToString() !="")
                        {
                            decimal poidsNet = Convert.ToDecimal(gvLigneEdit.GetListSourceRowCellValue(e.ListSourceRowIndex, "DL_PoidsNet"));
                            decimal prix = Convert.ToDecimal(txt_prix.Text);
                            decimal poids = Convert.ToDecimal(txt_poids.Text);
